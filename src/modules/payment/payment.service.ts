...
export class PaymentService {
  ...
  async createPayment(payload: CreatePayemntDto) {
    ...
    const value = await this.calculate(payload.comboType, payload.numberCombo)
    ...
    var ipnUrl = 'https://c157-14-186-69-61.ngrok-free.app/v1/payment/callback';
  ...
  async paySuccessful(orderId: String) {
    ...
    const student = await this.studentRepository.findOne({
      where: {
        ssoId: payment.studentId,
      }
    })

    student.boughtPaper += payment.numberOfPages;
    student.save();
    payment.save();
