...
export class ValidateSearchingInputPipe<
  T extends object,
> extends ValidationPipe {
      ...
      for (const criterion of criteria) {
        ...
        if (!this.isOperatorValidForType(field, operator)) {
          throw new UnprocessableEntityException(
            `Toán tử "${operator}" không tương thích với trường "${field}" với giá trị "${fieldValue}".`,
          );
        }

        const partialEntity = plainToClass(this.options, {
          [field]: fieldValue,
        });

        delete (partialEntity as any).lang;

        const errors = await validate(partialEntity, {
          whitelist: true,
  ...
  private isOperatorValidForType(field: string, operator: string): boolean {
    ...
    switch (fieldType) {
      ...
      case Number:
        return ['=', '!=', '<', '<=', '>', '>=', 'isSet', 'isNotSet'].includes(
          operator,
        );
      case Boolean:
        return ['=', '!=', 'isSet', 'isNotSet'].includes(operator);
